...
{
    internal sealed class TokenService : ITokenInterface
    {

        private IDictionary<string, AuthResponse> _contracts = new Dictionary<string, AuthResponse>()
        {
        ...
        public TokenResponse Authenticate(string user, string password)
        {
            ...
            int i = 0;
            foreach (KeyValuePair<string, AuthResponse> pair in _contracts)
            {
                i++;
